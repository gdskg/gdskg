...
def test_index_folder(test_dirs):
    ...
    # Test indexing
    result = index_repository(str(repo_path), overwrite=True)
...
def test_query_knowledge_graph(test_dirs):
    ...
    index_repository(str(repo_path), overwrite=False) # Append
    
    result = query_knowledge_graph("world")
...
def test_index_with_plugins(test_dirs):
    ...
    result = index_repository(str(repo_path), overwrite=True, plugins=["NonExistentPlugin"])
    ...
    result = index_repository(str(repo_path), overwrite=True, plugins=["GitHubPR"], parameters=["GitHubPR:key=value"])
    assert "Successfully indexed" in result
